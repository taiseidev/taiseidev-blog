...
> **このサイトは写真ポートフォリオです。** トップページに表示されるのは
> `category: photo` かつ `photos:` を持つ記事だけです（`photos:` の1枚目が
> シリーズのカバーになり、一覧では 3:2 にクロップされて表示されます）。
> それ以外のカテゴリの記事は URL 直打ちでは見られますが、一覧には出ません。
